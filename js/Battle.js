...
Battle.prototype = {
    ...
    round: function () {
        ...
        var attackerRoundOne = function (attacker, defender) {
            ...
            if (percentChance() < attackerHitChance) {
                ...
                if (percentChance() < attackerCriticalChance) {
                    defender.HP -= attackerDamage * 2;
                    defender.normaliseHP();
                    logToBattle("Attacker " + attacker.name + " scores a critical hit for " + attackerDamage * 2 + " damage!");
                } else {
        ...
        var defenderRoundOne = function (attacker, defender) {
            ...
            if (percentChance() < defenderHitChance) {
                ...
                if (percentChance() < defenderCriticalChance) {
                    attacker.HP -= defenderDamage * 2;
                    attacker.normaliseHP();
                    logToBattle("Defender " + defender.name + " scores a critical hit for " + defenderDamage * 2 + " damage!");
                } else {
        ...
        if (this.attacker.terrain.heal > 0) {
            healing = Math.floor(this.attacker.terrain.heal);
        ...
        if (this.defender.terrain.heal > 0) {
            healing = Math.floor(this.defender.terrain.heal);
